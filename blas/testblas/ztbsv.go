...
func ztbsvTest(t *testing.T, impl Ztbsver, rnd *rand.Rand, uplo blas.Uplo, trans blas.Transpose, diag blas.Diag, n, k, ldab, incX int) {
	...
	if !zsame(ab, abCopy) {
		t.Errorf("Case uplo=%v,trans=%v,diag=%v,n=%v,k=%v,ldab=%v,incX=%v: unexpected modification of A", uplo, trans, diag, n, k, ldab, incX)
	}
	if !zSameAtNonstrided(x, want, incX) {
		t.Errorf("Case uplo=%v,trans=%v,diag=%v,n=%v,k=%v,ldab=%v,incX=%v: unexpected modification of x\nwant %v\ngot  %v", uplo, trans, diag, n, k, ldab, incX, want, x)
	}
	if !zEqualApproxAtStrided(x, want, incX, tol) {
		t.Errorf("Case uplo=%v,trans=%v,diag=%v,n=%v,k=%v,ldab=%v,incX=%v: unexpected result\nwant %v\ngot  %v", uplo, trans, diag, n, k, ldab, incX, want, x)
	}
}
